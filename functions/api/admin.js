...
export async function onRequestPost(ctx) {
  try {
    ...
    if (password !== ctx.env.ADMIN_PASSWORD) {
      return json(
...
export async function onRequestGet(ctx) {
  try {
    ...
    const url =
      `${ctx.env.SUPABASE_URL}/rest/v1/payments` +
      `?select=id,order_id,name,email,amount,method,slip_path,status,certificate_id,created_at,approved_at` +
